	...
	render() {
		...
		return (
			<SidebarContext.Provider value={{ pinned: isPinned }}>
				<div
					className={cssNames('Sidebar flex column', className, {
						pinned: isPinned,
					})}>
					<div className='header flex align-center'>
						...
						<Icon
							className='pin-icon'
							material={
								isPinned ? 'keyboard_arrow_left' : 'keyboard_arrow_right'
							}
							onClick={toggle}
							tooltip={
								isPinned ? (
...
const navItemStorage = createStorage<[string, boolean][]>(
	'sidebar_menu_item',
	[]
);
...
class SidebarNavItem extends React.Component<SidebarNavItemProps> {
	...
	render() {
		...
		const extendedView =
			(subMenus.length > 0 || children) && this.context.pinned;
		if (extendedView) {
			const isActive = this.isActive();
			return (
				<div className={cssNames('SidebarNavItem', className)}>
					<div
						className={cssNames('nav-item', { active: isActive })}
					...
					<ul className={cssNames('sub-menu', { active: isActive })}>
						{subMenus.map(({ title, url }) => (
							<NavLink
								key={url}
								to={url}
								className={cssNames({ visible: this.isExpanded })}>
								{title}
							</NavLink>
						))}
						{React.Children.toArray(children).map(
							(child: React.ReactElement<any>) => {
								return React.cloneElement(child, {
									className: cssNames(child.props.className, {
		...
		return (
			<NavLink
				className={cssNames('SidebarNavItem', className)}
				to={url}
